-- La formula lunga appartiene alla sola finalissima del tabellone principale.
-- next_game_id non identifica la finale nei tabelloni costruiti manualmente:
-- quei match nascono tutti scollegati. La partita decisiva viene quindi scelta
-- per ultimo turno e prima posizione del tabellone principale.
...
begin
  ...
  select candidate.id into final_game_id
  from public.tournament_games candidate
  where candidate.tournament_id = game.tournament_id
    and candidate.phase = 'knockout'
    and candidate.bracket_no = 1
  order by candidate.round_no desc, candidate.position asc
  limit 1;

  is_final := game.id = final_game_id;
...
comment on function public.submit_tournament_result(uuid, smallint[], smallint[]) is
  'Registra il risultato applicando la formula lunga solo alla finalissima del tabellone principale.';
