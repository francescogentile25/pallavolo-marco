...
import { weatherIconName } from './weather-icon.model';

/**
 * Icona meteo. Con il movimento consentito usa le Meteocons, che sono SVG con
 * l'animazione dentro il file: un <img> basta, niente librerie a runtime. Con
 * `prefers-reduced-motion` resta il disegno geometrico, fermo e leggibile.
 */
@Component({
  ...
  template: `
    @if (animated) {
      <img [src]="'assets/weather/' + iconName() + '.svg'" [alt]="label()" />
    } @else {
...
export class WeatherGlyph {
  ...
  protected readonly animated = motionAllowed();
  ...
  protected readonly iconName = computed(() => weatherIconName(this.code(), this.night()));
}
