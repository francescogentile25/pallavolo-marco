...
@Component({
    ...
    <main class="detail-page">
      ...
      @if (store.loading() && !store.selected()) {
        ...
        <header class="match-hero">
          ...
          <div class="date-block"><span>{{ match.starts_at | date: 'dd' }}</span><div><strong>{{ match.starts_at | date: 'EEEE MMMM' : undefined : 'it-IT' }}</strong><small>{{ match.starts_at | date: 'HH:mm' }} · {{ match.duration_minutes ? match.duration_minutes + ' minuti' : 'durata non definita' }}</small></div></div>
        </header>
        ...
        <div class="detail-grid">
          <section class="card" aria-labelledby="participants-title">
            ...
            <div class="participants">
              ...
              @for (slot of emptySlots(); track slot) {
                @if (canJoin()) {
                  <button type="button" class="empty-slot is-clickable" (click)="join()" [disabled]="busy()"><i class="pi pi-plus" aria-hidden="true"></i><span>Posto disponibile · iscriviti</span></button>
                } @else {
                  <div class="empty-slot"><i class="pi pi-plus" aria-hidden="true"></i><span>Posto disponibile</span></div>
                }
              }
            </div>
          </section>
          ...
          <section class="card info-card" aria-labelledby="info-title">
            ...
            <dl><div><dt><i class="pi pi-map-marker"></i> Indirizzo</dt><dd>{{ match.court.venue.address }}, {{ match.court.venue.city }}</dd></div><div><dt><i class="pi pi-users"></i> Formula</dt><dd>{{ genderLabels[match.gender] }}, {{ match.capacity }} giocatori</dd></div><div><dt><i class="pi pi-chart-line"></i> Livello</dt><dd>{{ levelRange(match) }}</dd></div><div><dt><i class="pi pi-sun"></i> Campo</dt><dd>{{ match.court.indoor ? 'Coperto' : 'Scoperto' }}</dd></div></dl>
          ...
          <section class="card feedback-card" aria-labelledby="feedback-title">
            ...
            @if (feedbackOpen()) {
              ...
              <div class="feedback-wait">
                ...
                <div>
                  <strong>Partita ancora in corso</strong>
                  <p>I voti si aprono quando l’organizzatore conclude la partita: da quel momento hai 7 giorni per valutare i compagni.</p>
                  @if (canClose()) { <p-button size="small" label="Concludi partita" [loading]="busy()" icon="pi pi-flag-fill" (onClick)="close()" /> }
                </div>
              </div>
            }
            <div class="feedback-grid">@for (participant of feedbackParticipants(); track participant.profile_id) {
              <app-match-feedback-card [participant]="participant" [locked]="!feedbackOpen()" [busy]="store.feedbackProfileId() === participant.profile_id" [canReportNoShow]="canReportNoShow(participant.profile_id)" (rating)="rate(participant.profile_id, $event)" (noShow)="reportNoShow(participant.profile_id, $event)" />
  ...
  `,
  styles: `
    .chat-open { display: inline-flex; align-items: center; gap: 8px; margin-top: 12px; padding: 10px 16px; color: var(--color-brand-strong); border: 1px solid var(--color-border); border-radius: var(--radius); background: var(--color-surface); font: inherit; font-size: .8rem; font-weight: 750; cursor: pointer; }
    ...
    .feedback-wait { display: flex; gap: 12px; padding: 14px; margin: -6px 0 16px; border: 1px solid var(--color-border); border-radius: var(--radius-lg); background: var(--color-brand-soft); }
    .feedback-wait > i { color: var(--color-brand-strong); font-size: 1.1rem; }
    .feedback-wait strong { font-size: .82rem; }
    .feedback-wait p { margin: 5px 0 0; color: var(--color-ink-muted); font-size: .76rem; line-height: 1.5; }
    .feedback-wait p-button { display: inline-flex; margin-top: 10px; }
    :host { display: block; }.detail-page { width: min(100%, 1040px); padding: 18px 16px calc(var(--bottom-nav-height) + var(--bottom-actions-height) + 80px); margin: 0 auto; }.back-link { display: inline-flex; min-height: 44px; align-items: center; gap: 8px; color: var(--color-brand-strong); font-size: .78rem; font-weight: 850; text-decoration: none; }.match-hero { padding: 22px; color: white; border-radius: 8px; background: radial-gradient(circle at 90% 0, rgb(25 199 181 / .55), transparent 42%), linear-gradient(145deg, #0f1b23, #1d2b33); box-shadow: 0 18px 38px rgb(7 29 38 / .16); }.hero-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 34px; font-size: .72rem; font-weight: 850; }.status { padding: 5px 8px; border-radius: var(--radius); color: var(--color-brand-strong); background: #dffffa; text-transform: uppercase; }.status-full,.status-cancelled { color: var(--color-danger); background: var(--color-danger-soft); }.eyebrow { margin: 0 0 6px; color: var(--color-brand); font-size: .68rem; font-weight: 900; letter-spacing: .1em; text-transform: uppercase; }.match-hero .eyebrow { color: #a3b3fb; }.match-hero h1 { margin: 0 0 5px; font: 900 clamp(2rem, 10vw, 4.3rem)/.95 var(--display-font); letter-spacing: -.05em; }.match-hero > p:not(.eyebrow) { margin: 0; color: rgb(255 255 255 / .7); }.date-block { display: flex; align-items: center; gap: 12px; padding-top: 18px; margin-top: 20px; border-top: 1px solid rgb(255 255 255 / .15); text-transform: capitalize; }.date-block > span { font: 900 2.5rem/1 var(--display-font); }.date-block div { display: grid; gap: 3px; }.date-block small { color: rgb(255 255 255 / .68); }.public-banner { display: flex; align-items: center; gap: 10px; margin-top: 14px; padding: 12px 14px; color: var(--color-brand-strong); border: 1px solid var(--color-brand); border-radius: var(--radius); background: var(--color-brand-soft); font-size: .8rem; }.public-banner i { font-size: 1rem; }.detail-grid { display: grid; gap: 14px; margin-top: 14px; }.card { padding: 20px; border: 1px solid var(--color-border); border-radius: var(--radius-lg); background: var(--color-surface); }.section-heading { display: flex; align-items: end; justify-content: space-between; margin-bottom: 16px; }.section-heading h2 { margin: 0; font: 900 1.45rem/1 var(--display-font); }.section-heading > strong { color: var(--color-brand-strong); }.participants { display: grid; gap: 8px; }.participant,.empty-slot { display: flex; min-height: 58px; align-items: center; gap: 11px; padding: 8px; border-radius: var(--radius-lg); background: var(--color-surface-muted); }.avatar { display: grid; width: 42px; height: 42px; flex: 0 0 42px; place-items: center; overflow: hidden; border-radius: var(--radius); color: white; background: var(--color-brand-strong); font-size: .7rem; font-weight: 900; }.avatar img { width: 100%; height: 100%; object-fit: cover; }.participant > div:nth-child(2) { display: grid; min-width: 0; }.participant small { color: var(--color-ink-muted); font-size: .68rem; }.participant > i { margin-left: auto; color: var(--color-tournament); }.empty-slot { color: var(--color-ink-muted); border: 1px dashed var(--color-border); background: transparent; font-size: .75rem; }.empty-slot.is-clickable { width: 100%; font: inherit; font-size: .75rem; font-weight: 750; cursor: pointer; }.empty-slot.is-clickable:hover { color: var(--color-brand-strong); border-color: var(--color-brand); background: var(--color-brand-soft); }.empty-slot.is-clickable:disabled { cursor: default; opacity: .6; }.empty-slot i { display: grid; width: 42px; height: 42px; place-items: center; border-radius: var(--radius); background: var(--color-surface-muted); }.info-card dl { display: grid; gap: 15px; margin: 0; }.info-card dl div { display: grid; gap: 4px; }.info-card dt { color: var(--color-ink-muted); font-size: .68rem; font-weight: 800; }.info-card dt i { width: 20px; color: var(--color-brand-strong); }.info-card dd { margin: 0 0 0 24px; font-size: .82rem; font-weight: 750; }.notes { padding: 14px; margin-top: 18px; border-radius: var(--radius); background: var(--color-brand-soft); }.notes strong { font-size: .72rem; }.notes p { margin: 6px 0 0; font-size: .78rem; line-height: 1.5; white-space: pre-wrap; }.mobile-cta { display: grid; gap: 8px; padding-top: 14px; }.mobile-cta a[pButton] { justify-content: center; text-decoration: none; }.state { display: grid; min-height: 60dvh; place-content: center; justify-items: center; gap: 10px; color: var(--color-ink-muted); text-align: center; }.spinner { width: 20px; height: 20px; border: 2px solid currentColor; border-right-color: transparent; border-radius: 50%; animation: spin .7s linear infinite; }@keyframes spin { to { transform: rotate(360deg); } }a:focus-visible { outline: 3px solid var(--color-focus); outline-offset: 2px; }@media (min-width: 768px) { .detail-page { padding: 34px 28px 120px; }.match-hero { padding: 34px; }.detail-grid { grid-template-columns: minmax(0, 1.2fr) minmax(280px, .8fr); }.mobile-cta { display: none; } }
  `,
})
export class MatchDetail implements OnInit, OnDestroy {
  ...
  protected readonly canClose = computed(() => { const m = this.store.selected(); return !!m && m.creator_id === this.authStore.authUser()?.id && ['open','full','in_progress'].includes(m.status) && Date.now() >= new Date(m.starts_at).getTime(); });
  ...
  /** I voti li accetta solo il database a partita conclusa: prima si vedono, ma restano chiusi. */
  protected readonly feedbackOpen = computed(() => this.store.selected()?.status === 'completed');
  protected readonly showFeedback = computed(() => { const m = this.store.selected(); return !!m && ['in_progress', 'completed'].includes(m.status) && this.joined(); });
