      ...
      @if (store.loading() && !store.selected()) {
        ...
        @if (showFeedback()) {
          <section class="card feedback-card" aria-labelledby="feedback-title">
            ...
            <p class="feedback-help">Valuta il livello dei compagni: si vota dal fischio d’inizio e c’è tempo 7 giorni. Il voto è definitivo.@if (!feedbackOpen()) { Il no-show si segnala dopo aver concluso la partita.}</p>
...
export class MatchDetail implements OnInit, OnDestroy {
  ...
  /** Partita conclusa: solo da qui in poi si possono segnalare le assenze. */
