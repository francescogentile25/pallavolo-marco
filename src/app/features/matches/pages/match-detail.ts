...
@Component({
  ...
  imports: [Button, DatePipe, RouterLink, MatchFeedbackCard],
...
export class MatchDetail implements OnInit, OnDestroy {
  ...
  private readonly route = inject(ActivatedRoute); private readonly actions = inject(PageActionsService); private readonly service = inject(MatchesService);
  ...
  constructor() { effect(() => { if (this.canJoin()) this.actions.set([{ id:'join-match', label:'Iscriviti alla partita', shortLabel:'Iscriviti', icon:'pi-user-plus', primary:true, click:()=>void this.join() }]); else if (this.canWithdraw()) this.actions.set([{ id:'withdraw-match', label:'Ritirati dalla partita', shortLabel:'Ritirati', icon:'pi-user-minus', danger:true, click:()=>void this.withdraw() }]); else if (this.canCancel()) this.actions.set([{ id:'cancel-match', label:'Annulla partita', shortLabel:'Annulla', icon:'pi-times', danger:true, click:()=>void this.cancel() }]); else if (this.canClose()) this.actions.set([{ id:'close-match', label:'Concludi partita', shortLabel:'Concludi', icon:'pi-flag-fill', primary:true, click:()=>void this.close() }]); else this.actions.clear(); }); }
  ...
  protected async withdraw(): Promise<void> { if (window.confirm('Vuoi liberare il tuo posto?')) await this.store.withdraw(this.matchId); }
  protected async cancel(): Promise<void> { if (window.confirm('Annullare la partita per tutti i partecipanti?')) await this.store.cancel(this.matchId); }
  protected async close(): Promise<void> { if (window.confirm('Confermi che la partita è terminata?')) await this.store.close(this.matchId); }
  protected async rate(profileId: string, score: number): Promise<void> { if (window.confirm(`Inviare la valutazione ${score}/7? Non potrà essere modificata.`)) await this.store.rate(this.matchId, profileId, score); }
  protected async reportNoShow(profileId: string, reason: string): Promise<void> { if (window.confirm('Registrare il no-show? L’affidabilità del giocatore diminuirà.')) await this.store.reportNoShow(this.matchId, profileId, reason); }
