...
@Component({
          ...
          <ol class="chat-list">
            @for (m of messages(); track m.id; let i = $index) {
              <li class="chat-msg" [class.is-mine]="mine(m)" [class.is-grouped]="grouped(i)">
                @if (m.deleted) {
                  <span class="chat-tomb">Messaggio eliminato</span>
                } @else {
                  <div class="chat-row">
  ...
  styles: `
    ...
    .chat-body { display: flex; flex-direction: column; height: min(56vh, 480px); overflow-y: auto; padding: 4px 2px; }
    ...
    .chat-tomb { display: inline-block; padding: 5px 12px; color: var(--color-ink-muted); border-radius: 14px; background: color-mix(in srgb, var(--color-ink) 5%, transparent); font-size: .74rem; font-style: italic; }
    .chat-row { position: relative; display: flex; flex-direction: column; max-width: 78%; }
    ...
    .chat-actions { position: relative; display: flex; gap: 2px; margin-top: 3px; opacity: 0; transition: opacity .15s; }
    .chat-row:hover .chat-actions, .chat-actions:focus-within { opacity: 1; }
    .chat-act { display: grid; width: 26px; height: 26px; place-items: center; color: var(--color-ink-muted); border: 0; border-radius: 8px; background: none; font-size: .72rem; cursor: pointer; }
    ...
    .chat-composer { display: grid; gap: 6px; padding-top: 10px; border-top: 1px solid var(--color-border); }
    ...
    .chat-act:focus-visible, .chat-send:focus-visible, .chat-react-pill:focus-visible, .chat-banner button:focus-visible { outline: 2px solid var(--color-focus); outline-offset: 2px; }
    @media (max-width: 640px) { .chat-row { max-width: 88%; } }
  `,
})
export class ChatDialog {
  ...
  protected aggregated(m: ChatMessage): AggregatedReaction[] {
    ...
    return REACTION_ORDER.map((type) => {
      ...
      return {
        ...
        count: list.length,
        mine: list.some((r) => r.user_id === uid),
      };
    }).filter((r) => r.count > 0);
  }
  ...
  protected askDelete(m: ChatMessage): void {
    this.confirm.confirm({
      ...
      message: 'Vuoi eliminare questo messaggio? Resterà una traccia "messaggio eliminato".',
      icon: 'pi pi-trash',
      acceptLabel: 'Elimina',
      rejectLabel: 'Annulla',
