...
export class TournamentsList implements OnInit, OnDestroy {
  ...
  protected readonly filtered = computed(() => { const query = this.query().trim().toLocaleLowerCase('it'); return this.store.tournaments().filter(item => (this.format() === 'all' || item.format === this.format()) && (this.visibility() === 'all' || item.visibility === this.visibility()) && (!query || `${item.title} ${item.venue.name} ${item.venue.city}`.toLocaleLowerCase('it').includes(query))); });
  ...
  protected resetFilters(): void { this.query.set(''); this.format.set('all'); this.visibility.set('all'); }
  private channel?: RealtimeChannel; private timer?: ReturnType<typeof setTimeout>;
  constructor() {
    effect(() => { this.query(); this.format(); this.visibility(); this.first.set(0); });
  }
