...
export const TournamentsStore = signalStore(
  ...
  withMethods((store, service = inject(TournamentsService), messages = inject(MessageService)) => {
    ...
    return {
      ...
      advanceBye: (id: string, gameId: string) => act(id, () => service.advanceBye(gameId), 'Squadra qualificata', 'Con il BYE passa senza giocare.'),
      ...
      setContacts: (id: string, email: string | null, phone: string | null) => act(id, () => service.setContacts(id, email, phone), 'Contatti aggiornati', 'Chi guarda il torneo sa a chi scrivere o telefonare.'),
