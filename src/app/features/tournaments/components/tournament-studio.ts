...
import {
  ...
  TournamentGame,
  TournamentGameDraft,
  TournamentPersonalPreset,
  TournamentPresetRules,
  TournamentTeam,
      ...
      @if (view() === 'rules') {
        <div class="rules-layout">
          <section class="rule-card">
            ...
            <footer class="rule-actions"><p-button label="Salva come preset" icon="pi pi-bookmark" severity="secondary" [outlined]="true" (onClick)="presetDialog.set(true)" /><p-button label="Applica le regole" icon="pi pi-check" [loading]="store.saving()" (onClick)="saveRules()" /></footer>
          </section>

          <aside class="preset-library">
            <header><div><span>I tuoi modelli</span><h3>Preset personali</h3></div><b>{{ store.presets().length }}</b></header>
            @for (preset of store.presets(); track preset.id) {
              <article [class.favorite]="preset.is_favorite">
                <button type="button" class="preset-main" (click)="applyPreset(preset)"><i class="pi pi-bookmark"></i><span><strong>{{ preset.name }}</strong><small>{{ preset.description || presetSummary(preset) }}</small></span></button>
                <div><p-button [icon]="preset.is_favorite ? 'pi pi-star-fill' : 'pi pi-star'" [text]="true" severity="secondary" [ariaLabel]="preset.is_favorite ? 'Rimuovi dai preferiti' : 'Aggiungi ai preferiti'" (onClick)="favorite(preset)" /><p-button icon="pi pi-trash" [text]="true" severity="danger" ariaLabel="Elimina preset" (onClick)="confirmDeletePreset(preset)" /></div>
              </article>
            } @empty { <div class="preset-empty"><i class="pi pi-bookmark"></i><p>Salva le tue regole per riutilizzarle nei prossimi tornei.</p></div> }
          </aside>
        </div>
      }
    </section>
...
export class TournamentStudio implements OnInit {
  ...
  protected readonly presetDialog = signal(false);
  protected readonly presetName = signal('');
  protected readonly presetDescription = signal('');
  protected readonly rules = signal<TournamentPresetRules>(this.emptyRules());
  ...
  protected readonly studioViews = [{ value: 'groups' as const, label: 'Gironi', icon: 'pi pi-users' }, { value: 'bracket' as const, label: 'Tabellone', icon: 'pi pi-sitemap' }, { value: 'rules' as const, label: 'Regole e preset', icon: 'pi pi-sliders-h' }];
  ...
  ngOnInit(): void { this.rules.set(this.rulesFromTournament(this.tournament())); void this.store.loadPresets(); }
  ...
  protected updateRule<K extends keyof TournamentPresetRules>(key: K, value: TournamentPresetRules[K]): void { this.rules.update(rules => ({ ...rules, [key]: value })); }
  protected saveRules(): void { void this.store.updateRules(this.tournament().id, this.rules()); }
  protected applyPreset(preset: TournamentPersonalPreset): void { this.rules.set({ ...preset.rules, preset: 'custom' }); }
  protected presetSummary(preset: TournamentPersonalPreset): string { return `${preset.rules.maxTeams} coppie · ${preset.rules.format}`; }
  protected async favorite(preset: TournamentPersonalPreset): Promise<void> { await this.store.favoritePreset(preset.id, !preset.is_favorite); }
  protected confirmDeletePreset(preset: TournamentPersonalPreset): void { this.confirmation.confirm({ header: 'Elimina preset', message: `“${preset.name}” verrà rimosso solo dalla tua raccolta.`, icon: 'pi pi-bookmark', acceptLabel: 'Elimina', rejectLabel: 'Annulla', acceptButtonProps: { severity: 'danger' }, accept: () => void this.store.deletePreset(preset.id) }); }
  protected async savePreset(): Promise<void> { if (await this.store.savePreset(this.presetName().trim(), this.presetDescription().trim() || null, this.rules())) { this.presetDialog.set(false); this.presetName.set(''); this.presetDescription.set(''); } }
  private rulesFromTournament(item: Tournament): TournamentPresetRules { return { preset: item.preset, registrationMode: item.registration_mode, format: item.format, maxTeams: item.max_teams, guaranteedMatches: item.guaranteed_matches, groupSize: item.group_size, qualifiersPerGroup: item.qualifiers_per_group, groupBestOf: item.group_best_of, groupSetPoints: item.group_set_points, knockoutBestOf: item.knockout_best_of, knockoutSetPoints: item.knockout_set_points, tiebreakPoints: item.tiebreak_points, winByTwo: item.win_by_two, thirdPlace: item.third_place, standingsWinPoints: item.standings_win_points, standingsLossPoints: item.standings_loss_points, minimumRestMinutes: item.minimum_rest_minutes, resultConfirmationRequired: item.result_confirmation_required }; }
  private emptyRules(): TournamentPresetRules { return { preset: 'custom', registrationMode: 'hybrid', format: 'mixed', maxTeams: 12, guaranteedMatches: 0, groupSize: 4, qualifiersPerGroup: 2, groupBestOf: 1, groupSetPoints: 21, knockoutBestOf: 3, knockoutSetPoints: 21, tiebreakPoints: 15, winByTwo: true, thirdPlace: false, standingsWinPoints: 2, standingsLossPoints: 0, minimumRestMinutes: 15, resultConfirmationRequired: false }; }
}
